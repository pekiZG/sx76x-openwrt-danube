...
#if (VIN_CFG_FEATURES & VIN_FEAT_FAX_T38)
IFX_LOCAL IFX_int32_t FaxSetupConf (TAPI_CONNECTION * pChannel);
#endif /* (VIN_CFG_FEATURES & VIN_FEAT_FAX_T38) */
#ifdef TAPI_CID
...
Arguments:
  pChannel  - pointer to TAPI_CONNECTION structure
Return:
   IFX_SUCCESS or IFX_ERROR
Remark :
...
#endif /* (VIN_CFG_FEATURES & VIN_FEAT_VOICE) */
   /* set near end lec coefficients and disable NLP on associated
      phone channel */
   if (ret == IFX_SUCCESS)
   {
      VINETIC_CHANNEL *pVinCh;
      IFX_uint8_t      phone_ch;

      /* get instance of phone channel */
      TAPI_Data_Get_Phone_Channel (pChannel, &phone_ch);
      pVinCh   = &pDev->pChannel[phone_ch];
      /* do ALM operations with a phone channel pointer */
      ret = Dsp_SetNeLec_Alm (pVinCh, IFX_TRUE, IFX_FALSE);
   }
...
Return:
   IFX_SUCCESS/IFX_ERROR
Remarks:
   - CID operations take place in the DSP. Therefore, the caller must make sure
     that a data channel pointer is passed from Tapi.
...
\todo
   ...
   generator, ATD, UTD. For ETU Sig-Ch and no generator
   Switch connected signaling channel on and off: store all en bits.
   Sig-Ch, ATD, UTD, DTMF receiver and generator (UTG and CID should be not
   active).
...
\todo
   On the fly changing while coder is running is possible:
   SSRC, timestamp, Seq-Nr, nEvents, nPlayEvents. The latter two are applied
   when the coder is switched on -> remember settings and apply on rec start.
   nEventPT is only applied when coder is switched off
*******************************************************************************/
                                   ...
                                   TAPI_RTP_CONF const *pRtpConf)
{
...
#endif /* 0 */
   {
      ...
      {
      case TAPI_RTPEVPLAY_DEFAULT:
      case TAPI_RTPEVPLAY_PLAY:
         /* all playout always enabled */
         sigConf.bit.evMaskTOG = 0x7;
         break;
      case TAPI_RTPEVPLAY_MUTE:
         /* all playout always enabled */
...
Remarks:
\todo
   Recommend to apply before starting coder
*******************************************************************************/
                                      ...
                                      TAPI_DATA_MAPPING const *pMap)
{
   ...
   err = Con_ConnectPrepare (&pDev->pSigCh[nThisCh].signal, pDstMod,
                          ALM_TO_SIG_IN);
   /* check if signaling module is already connected. If yes this is a local
      conference (one coder, two ALM/PCM), so the coder and ALM or PCM
      are connected */
   if (err == IFX_SUCCESS)
   {
   ...
   if (err == IFX_SUCCESS)
   {
      /* get the number of signaling modules and the list of it connected
         to this module. The list is needed in case of conferencing, where
         each signaling channel output must be connected to each coder */
      pInput = pDstMod->pInputs;
      while (pInput != IFX_NULL)
      {
      ...
      if (cnt > 1)
      {
         /* conference with two external. So now connect each signaling
           output to every coder. ConnectPrepare maybe called although
           the input is already connected */
         for (i = 0; i <  cnt; i++)
         {
            for (j = 0; j < cnt; j++)
            {
               /* check if SigOutA is already connected (drivers default connection) */
               if (tmpSigs[i]->pInputs->pParent != tmpSigs[j]->pInputs->pParent)
                  /* this must be added to all other connected channels. We add SigOutB */
                                         ...
                                         TAPI_DATA_MAPPING const *pMap)
{
   VINETIC_DEVICE *pDev = (VINETIC_DEVICE *) pChannel->pDevice;
   IFX_uint32_t nThisCh = pChannel->nChannel;
   IFX_int32_t  err = IFX_SUCCESS, i, cnt = 0;
   ...
   {
      /********************************************************************/
      /****** following code is only for two external and one local *******/
      /********************************************************************/
      /* Remove our signaling output from the other coders.
         Note: at pInputs2 all other modules except our coder are
         connected in a conference with two externals */
      pInput = pDev->pSigCh[nThisCh].signal.pInputs2;
      while (pInput != IFX_NULL)
      {
         /* check if it is a coder. the ALM is disconnected at the next step */
         if (pInput->pParent->nModType == VINDSP_MT_COD)
            err = Con_DisconnectPrepare (&pDev->pSigCh[nThisCh].signal,
                                       pInput->pParent, ALM_TO_SIG_IN);
         pInput = pInput->pNext;
      }
      if (err == IFX_SUCCESS)
      {
         /* remove all inputs from the coder except our signaling module */
         for (i = 0; i <  MAX_MODULE_SIGNAL_INPUTS; i++)
         {
            pTmpMod = pDev->pCodCh[nThisCh].signal.in[i].pOut;
            if (pTmpMod != IFX_NULL &&
               pTmpMod != &pDev->pSigCh[nThisCh].signal)
   ...
   {
      /* Vice versa: disconnect SIG from Dst */
      err = Con_DisconnectPrepare (pDstMod, &pDev->pSigCh[nThisCh].signal, 0);

      if (cnt < 2)
      {
         /********************************************************************/
         /****** following code is only for one external and two local *******/
         /********************************************************************/
                                     ...
                                     TAPI_PCM_MAPPING const *pMap)
{
   ...
   err = Con_ConnectPrepare (&pDev->pPcmCh[nThisCh].signal,
                          &pDev->pAlmCh[nDstCh].signal, 0);
   /* 2. connect ALM out to PCM directely */
   if (err == IFX_SUCCESS)
      err = Con_ConnectPrepare (&pDev->pAlmCh[nDstCh].signal,
                                        ...
                                        TAPI_PCM_MAPPING const *pMap)
{
   ...
   err = Con_DisconnectPrepare (&pDev->pPcmCh[nThisCh].signal,
                             &pDev->pAlmCh[nDstCh].signal, 0);
   /* 2. disconnect ALM out to PCM directely */
   if (err == IFX_SUCCESS)
      err = Con_DisconnectPrepare (&pDev->pAlmCh[nDstCh].signal,
                             &pDev->pPcmCh[nThisCh].signal, 0);
   if (err == IFX_SUCCESS)
      err = Con_ConnectConfigure (pDev);
   else
...
Remarks:
   ...
   -  The configuration of the Demodulator requires that the datapump is
      disable. So the followings are done here :
         1- Disable datapump if enable
         2- Set Demodulator
...
IFX_int32_t TAPI_LL_FaxT38_DisableDataPump (TAPI_CONNECTION * pChannel)
{
   ...
   /* Enable NLP on associated phone channel */
   ...
   if (ret == IFX_SUCCESS)
   {
      ...
      pVinCh   = &pDev->pChannel[phone_ch];
      /* Do ALM operations with a phone channel pointer */
      ret = Dsp_SetNeLec_Alm (pVinCh, IFX_TRUE, IFX_TRUE);
   }
...
\todo
   Store ET. If coder is running it is programmed otherwise it is programmed
   when coder is started.
*******************************************************************************/
...
\todo SIC maybe modified in G.711, G.723 and ILBC on fly.
