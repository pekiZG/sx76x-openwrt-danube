 ...
 ****************************************************************************
   ...
   Remarks     :
      ...
      related and assumes a data channel file descriptor.
...
IFX_LOCAL IFX_void_t cid2_timercallback (Timer_ID Timer, IFX_int32_t nArg)
{
   ...
   case  TAPI_CID2_CHECK_ACK:
      ...
      if (pCid2->bAck == IFX_TRUE)
      {
         ...
         ret = TAPI_Phone_Set_Linefeed (pCid2->pPhoneCh, TAPI_LINEFEED_NORMAL);
         if (ret == IFX_SUCCESS)
         {
   ...
   if (cleanUp == IFX_TRUE)
   {
      ...
      TAPI_Phone_Set_Linefeed (pCid2->pPhoneCh, TAPI_LINEFEED_NORMAL);
...
Return:
   IFX_SUCCESS or IFX_ERROR
Remarks:
   If Cid generation with FSK is chosen, low level configuration of
   CID transmitter will be done.
...
{
   TAPI_CONNECTION *pPhoneCh = IFX_NULL;
   IFX_uint8_t      phone_ch;
   IFX_int32_t      ret = IFX_SUCCESS;

   /* get phone channel on which CID1 is processed */
   TAPI_Data_Get_Phone_Channel (pChannel, &phone_ch);
   pPhoneCh = &pChannel->pTapiDevice->pChannelCon[phone_ch];
   /* This line shouldn't be ringing or sending cid 1 because
      cid configuration is affected here */
   if (pPhoneCh->TapiRingData.bRingingMode == IFX_TRUE)
   {
...
IFX_int32_t TAPI_Phone_CID_Send (TAPI_CONNECTION *pChannel, Timer_ID Timer)
{
   ...
   if (Timer != NULL)
   {
      ...
      case TAPI_CID_DTMF_MODE:
         nTime +=   ((pDtmfCid->digitTime + pDtmfCid->interDigitTime)
                  * pCidData->nCidParamLen);
         break;
      default:
      ...
      /* Reschedule the timer if CID1 is send before ring  burst */
      if (pCidData->nCidType == TAPI_CID_TYPE1)
      {
         if ((pCidConf->nAppearance == CID_BEFORE_RINGBURST) ||
             (pCidConf->nSpec == TAPI_CID_NTT))
         {
            TAPI_SetTime_Timer(Timer, nTime, IFX_FALSE, IFX_FALSE);
         }
         else if (pCidData->nAdditionalTime >= nTime)
         {
            /* Delay time is sufficient for sending cid. use it.
               Otherwise, use time calculated which ensures a CID transmission */
            if (nTime < pCidData->nAdditionalTime)
      ...
      else /* CID type 2 : no timings checked here */
      {
         nTime += pCidData->nAdditionalTime;
         /* reset additional time. caller must set it new if necessary */
         pCidData->nAdditionalTime = 0;
...
Arguments:
   ...
   nOperation  - cid1 flow operation: alert/send/ring
   timerId     - timer of which cid operation is attached
   alert       - function pointer to alert routine
   cidtxend    - function pointer to after cid routine
Return:
   IFX_SUCCESS/IFX_ERROR
Remark:
   This function is called from ringing machine to manage the flow of CID1
   sequence. Therefore, the channel refers to a phone channel and the data
   channel is used for the CID transmission itself.
*******************************************************************************/
                                 ...
                                 IFX_int32_t (*cidtxend)(TAPI_CONNECTION*))
{
   ...
   case  TAPI_CID1_SEND:
      ...
      if (pCidData->bIsNewData == IFX_TRUE)
      {
         ...
         /* Pass timer id and send CID .
            Note: If the timer is valid, it will be reschedule within
                  this function
         */
         ret = TAPI_Phone_CID_Send (pRingData->pDataCh, timerId);
      }
      else
      {
      ...
      break;
   case  TAPI_CID1_END:
      if (pCidData->nCidTxState != CID_TX_END)
      {
...
Remark:
   - If a non supported service is set, error will be returned.
   - This implementation complies with ETSI standards
   - This operation takes place on a data channel
*******************************************************************************/
                                    ...
                                    TAPI_CID_INFO_t const *pCidInfo)
{
   ...
   if (ret == IFX_SUCCESS)
   {
      if (pCidInfo->txMode == TAPI_CID_OFFHOOK)
         pChannel->TapiCid2.bCid2IsOn = IFX_TRUE;
      else
         pChannel->TapiCid2.bCid2IsOn = IFX_FALSE;
      /* send CID now */
...
Return:
   IFX_SUCCESS or IFX_ERROR
Remark
   - This operation is done on a data channel. In case any phone specific task
     should be done in the process (i.e Line Mode), care must be taken that
     phone task are done on the phone channel mapped to this data channel.
   -  The (un)mapping of any phone channel to any data channel is a user task.
*******************************************************************************/
IFX_int32_t TAPI_Phone_CID2_Send (TAPI_CONNECTION *pChannel, TAPI_PHONE_CID const *pPhoneCid)
{
   ...
   /* set phone channel on which CID2 is processed */
   TAPI_Data_Get_Phone_Channel (pChannel, &phone_ch);
   pCid2->pPhoneCh = &pChannel->pTapiDevice->pChannelCon[phone_ch];
   /* This line shouldn't be ringing or sending cid 1 */
   if (pCid2->pPhoneCh->TapiRingData.bRingingMode == IFX_TRUE)
   {
      TRACE(TAPI_DRV, DBG_LEVEL_HIGH, ("\n\rDRV_ERROR: Line is ringing\n\r"));
      return IFX_ERROR;
   }
    ...
    */
   if (pCid2->pPhoneCh->TapiOpControlData.nLineMode != TAPI_LINEFEED_NORMAL)
   {
