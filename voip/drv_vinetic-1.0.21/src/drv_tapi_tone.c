                                 ...
                                 IFX_int32_t nToneIndex, TAPI_TONE_DST dst)
{
   ...
   src = nToneIndex & (TAPI_TONE_SRC_TG | TAPI_TONE_SRC_DSP);
   nToneIndex &= ~(TAPI_TONE_SRC_TG | TAPI_TONE_SRC_DSP);
   ...
   if (pChannel->TapiTgToneData.nOnTime == 0 &&
       nToneIndex < TAPI_TONE_MIN_INDEX && nToneIndex > 0)
   {
      TRACE (TAPI_DRV, DBG_LEVEL_HIGH,
            ("\n\rDRV_ERROR: no on-time set for channel %d\n\r",
             pChannel->nChannel));
      return IFX_ERROR;
   }
   ...
   if (nToneIndex >= TAPI_TONE_MIN_INDEX)
   {
   ...
   else
   {
      ...
      {
         /* No tone is currently beeing played */
         if (nToneIndex >= TAPI_TONE_MIN_INDEX)
         {
         ...
         {
            pToneCoeff->tone.nPredefined = nToneIndex;
            pToneCoeff->type = TAPI_TONE_TYPE_PREDEF;
         }
      ...
      {
         /* tone is currently playing, try low level to add the new tone
            or to select another resource */
         ...
         {
            TRACE(TAPI_DRV, DBG_LEVEL_HIGH,
                  ("DRV_ERROR: Invalid tone playing on this channel\n\r"));
            ret = IFX_ERROR;
         }
      }
   }
                                      ...
                                      TAPI_SIMPLE_TONE const *pSimpleTone)
{
   ...
   toneCoefficients[pSimpleTone->index].tone.simple = *pSimpleTone;
                                         ...
                                         TAPI_COMPOSED_TONE const *pComposedTone)
{
   ...
   if (pComposedTone->count == 0 || pComposedTone->count > TAPI_MAX_SIMPLE_TONES)
   {
      TRACE(TAPI_DRV, DBG_LEVEL_HIGH,
                                        ...
                                        TAPI_PHONE_CPTD const *signal)
{
   ...
   if (ret == IFX_SUCCESS)
   {
      ...
      pChannel->TapiMiscData.sigMask |= TAPI_SIGNAL_CPTD;
   }
   return ret;
}
