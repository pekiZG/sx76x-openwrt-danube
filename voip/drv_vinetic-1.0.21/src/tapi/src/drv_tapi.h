...
#if 0
   TAPI_HOOK_STATE_FLASH_OFFHOOK,
   /* phone is offhook, waiting for next dialing pulse but the
      flash hook and digit low time overlapped */
   TAPI_HOOK_STATE_PULSE_H_FLASH_CONFIRM,
#endif
...
typedef struct
{
   ...
   IFXOS_event_t              asEvt;
   /* handle to the phone channel used together with
      the cid data channel on which CID2 is processed. */
...
#ifdef TAPI_CID
   /* handle to the cid data channel used together with
      the phone channel on which ringing is processed. */
...
struct _TAPI_CONNECTION
{
   ...
   TAPI_VALIDATION_TIMES TapiHookFlashMakeTime;
   ...
   /* If this structure is used as a phone channel, keep track of
      connected dsp data channels in the following bitfield variable */
   IFX_uint32_t          nPhoneDataChannels;
   /* If this structure is used as a dsp data channel, keep track of
      connected phone channels */
   /* Dsp channels bitfield connected to this analog channel
      and which should be used for the CID sending */
