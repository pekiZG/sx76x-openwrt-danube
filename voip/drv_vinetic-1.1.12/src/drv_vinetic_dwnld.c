   ...
   */
   ...
   if (err == IFX_SUCCESS)
   {
      ...
      if (err == IFX_SUCCESS)
      {
         ...
         pDev->hostDev.nEdspVers = pData[2];
      }
...
IFX_int32_t Dwld_LoadFirmware (VINETIC_DEVICE *pDev, VINETIC_EDSP_FWDWLD *pEdsp)
{
   ...
   /* for Vinetic V.21 (non 2CPE), autodownload is mandatory */
   if (pDev->nChipMajorRev == VINETIC_V2x &&  pDev->nChipRev != VINETIC_2CPE_V21)
   ...
   {
      /* Load ROM firmware if required and chip isn't Vinetic 2.1
         or if chip is Vinetic 2CPE and no RAM firmware downloaded */
      if ((pDev->nChipMajorRev == VINETIC_V1x || pDev->nChipRev == VINETIC_2CPE_V21) &&
