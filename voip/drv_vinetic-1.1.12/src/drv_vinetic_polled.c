  ...
  \subsection ac About event handling
   ...
   1- The client can handle events in the polling task which is here high prior
      ...
      static int eventHandling (int *aFd, int evt)
      {
         ...
         {
            events.channels = 0;
            ioctl (aFd [i], IFX_TAPI_CH_STATUS_GET, (int)&events);
            if (events.hook & IFX_TAPI_LINE_HOOK_STATUS_HOOK)
            {
              ioctl(aFd [i], IFX_TAPI_LINE_HOOK_STATUS_GET, (int)&nHook);
              printf ("ch%d -> HookState: %s\n\r", i, ((nHook == 0) ?
                "ONHOOK" : "OFFHOOK"));
            }
            if (events.hook & IFX_TAPI_LINE_HOOK_STATUS_FLASH)
            {
              printf ("ch%d -> Flashhook detected \n\r", i);
            }
            if (events.line & IFX_TAPI_LINE_STATUS_GNDKEY)
            {
              printf ("ch%d -> ground key detected \n\r", i);
            }
            if (events.dialing & IFX_TAPI_DIALING_STATUS_DTMF)
            {
              if (events.digit == 0xb)
                 events.digit = 0;
...
static int VINETIC_IrqPollEventConf(VINETIC_DEVICE *pDev);
...
int VINETIC_IrqPollDevAddEvent(VINETIC_DEVICE *pDev)
{
   ...
   return VINETIC_IrqPollEventConf(pDev);
}
   ...
   */
      ...
      enable/disable the interrupts accordingly */
   {
      ...
      for (i = 0; i < vinPollStruct.numCurDevsEvents; i++)
      {
         ...
         if (VINETIC_IrqPollEventConf(pTmpDev) != IFX_SUCCESS)
            return IFX_ERROR;
      }
   }
...
int VINETIC_Poll_Down (const int nElements, void **ppBuffers)
{
   ...
   for (;;)
   {
      ...
      if (nFibxms >= packetLen)
      {
         ...
         pIr              = (IFX_uint16_t *) pDev->hostDev.pIntReg;
         pNwd             = pDev->hostDev.pNwd;
         ...
         if (dwn[devNo].mbxSize > 0)
         {
            ...
            {  /* write data **************************************************/
               if (activeDevs == 1)
                  WAIT_FOR_READY_FLAG;
               if (pDwn->mbxSize != 1)
                  *(pDev->hostDev.pNwd)   = *(pDwn->pCurPktPos++);
               else
                  *(pDev->hostDev.pEom)   = *(pDwn->pCurPktPos++);
      ...
      FOR_ALL_DEVICES                                          /* index devNo */
      {
         ...
         if (ups[devNo].mbxSize > 0)
         {
            ...
            {  /* read voice data *********************************************/
               ...
               if (pUps->mbxSize != 1)
                  *(pUps->pCurPktPos++)    = *(pDev->hostDev.pNwd);
               else
                  *(pUps->pCurPktPos++)    = *(pDev->hostDev.pEom);
