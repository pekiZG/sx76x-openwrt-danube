...
*/
static int addSigIn (MODULE_SIGNAL_INPUTS *pSigIns, IFX_uint8_t nSignal, IFX_uint8_t* pChg)
{
   int i;
...
\param
   pCh      - handle to TAPI_CONNECTION structure
\param
   mt       - specifies the module type in which the signal should be
              added. Options are: COD, ALM, SIG
\param
   nSignal  - specifies the signal to be added
\param
   pSigs    - handle to SIGNAL_ARRAY_INPUTS structure
\return
   IFX_SUCCESS or IFX_ERROR
\remarks
...
*/
int addModuleSigIn (VINETIC_CHANNEL *pCh, VINDSP_MT mt, IFX_uint8_t nSignal,
...
\remarks
   ...
   conferencing implementation.
*/
int removeModuleSigIn (VINETIC_CHANNEL *pCh, VINDSP_MT mt, IFX_uint8_t nSignal,
                       SIGNAL_ARRAY_INPUTS* pSigs)
{
/*   VINETIC_DEVICE     *pDev = pCh->pParent;
...
/**
   Write the configuration according to pSigs
\param pCh     - handle to channel structure
\param pSigs  - handle to input signal array
\return
   IFX_SUCCESS or IFX_ERROR
*/
