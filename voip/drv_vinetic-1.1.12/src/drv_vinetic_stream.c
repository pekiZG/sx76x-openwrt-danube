                             ...
                             IFX_int32_t count)
{
   ...
   err = pDev->hostDev.write (pDev, pData, wrcnt + 2);
   /* release locks */
   ...
   {
#ifdef TESTING
      IF_TEST (TC_VOIPSTREAM)
      {
         pDev->TestResult[pCh->nChannel - 1].stream.nTxCnt++;
      }
#endif /* TESTING */
      return count;
   }
                            ...
                            IFX_int32_t count)
{
   ...
   /* check payload type in case of RTP */
   if ((pDev->hostDev.nEdspVers & ECMD_VERS_EDSP_PRT) == ECMD_VERS_EDSP_PRT_RTP)
   {
      /* for dynamic event payload type assignment */
#if (VIN_CFG_FEATURES & VIN_FEAT_VOICE)
                            ...
                            IFX_int32_t count)
{
   ...
   IFX_uint16_t voice_box_len = 0, wrcnt = (count >> 1) + (count & 0x1);
   IFX_int32_t err = count;
   ...
   if (V2CPE_BOX_VLEN_WLEN_GET(voice_box_len) < wrcnt)
   {
   ...
   /* protect write access againts tasks and interrupts  */
   VIN_UNPROT_VOICE_MBX_WRITE (pDev, pFaxBuf, wrcnt);
   CHECK_HOST_ERR (pDev, {err = IFX_ERROR; goto EXIT_RELEASE;});
...
EXIT_RELEASE:
   if (err == IFX_ERROR)
   {
      ...
      TAPI_FaxT38_Event_Update (pCh->pTapiCh, 0, IFX_TAPI_FAX_T38_ERROR_WRITE);
                            ...
                            IFX_int32_t count)
{
   VINETIC_DEVICE* pDev = pCh->pParent;
   IFX_uint16_t* pFaxBuf;
   IFX_uint16_t nFIBXMS, wrcnt = (count >> 1) + (count & 0x1);
   ...
   {
      /* not enough space available in the mailbox : write 0 bytes */
      if ((nFIBXMS & FIBXMS_PBOX) < wrcnt)
         return 0;
      /* protect write access againts tasks and interrupts  */
      IFXOS_MutexLock (pDev->mbxAcc);
      Vinetic_IrqLockDevice(pDev);
      err = pDev->hostDev.write (pDev, pFaxBuf, wrcnt);
      /* release locks */
      Vinetic_IrqUnlockDevice(pDev);
      IFXOS_MutexUnlock (pDev->mbxAcc);
   }
                          ...
                          IFX_int32_t count)
{
   ...
   IFX_uint32_t    pos = 0;
   IFX_int32_t     len = 0, err = IFX_SUCCESS;
   ...
   if (Fifo_isEmpty (&pCh->rdFifo) == IFX_FALSE)
   {
      ...
      if (pFaxPacket != NULL)
      {
         ...
         IFXOS_CPY_KERN2USR (buf, &pFaxPacket->cmd1, 2);
         pos += 2;
         IFXOS_CPY_KERN2USR (&buf[pos], &pFaxPacket->cmd2, 2);
         pos += 2;
	    ...
	    printk("\nTAPI_FaxT38_Event_Update (pCh->pTapiCh, IFX_TAPI_FAX_T38_TX_OFF,IFX_TAPI_FAX_T38_ERROR_NONE);\n");
         }
      }
      else
      {
   ...
   {
      /* set error and issue tapi exception */
      TAPI_FaxT38_Event_Update (pCh->pTapiCh, 0, IFX_TAPI_FAX_T38_ERROR_READ);
      TRACE (VINETIC, DBG_LEVEL_HIGH,
                          ...
                          PACKET **pPacket)
{
   ...
   IFXOS_ASSERT (nCh < VINETIC_MAX_CH_NR);
