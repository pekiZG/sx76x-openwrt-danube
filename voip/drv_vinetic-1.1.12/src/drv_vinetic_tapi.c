...
/* temporary define the signaling-alm module connection as default till
 implementation with signaling-coder is finished */
/*#define VIN_SIGALM_FIXED*/
...
IFX_LOCAL IFX_int32_t AddCaps (VINETIC_DEVICE *pDev)
{
   ...
   if ((!((pDev->hostDev.nEdspVers & ECMD_VERS_FEATURES)
           == ECMD_VERS_FEATURES_C)                          ||
         ((pDev->hostDev.nEdspVers & ECMD_VERS_FEATURES)
         ...
         (pDev->nChipType != VINETIC_TYPE_S))
   {
      ...
      if ((pDev->hostDev.nEdspVers & ECMD_VERS_MV) == ECMD_VERS_MV_4)
      {
         switch (pDev->hostDev.nEdspVers & ECMD_VERS_FEATURES)
         {
      ...
      /* set coder channels */
      switch (pDev->hostDev.nEdspVers & ECMD_VERS_MV)
      {
      case ECMD_VERS_MV_8:
...
IFX_return_t TAPI_LL_Phone_Init(TAPI_CONNECTION *pChannel, IFX_TAPI_CH_INIT_t const *pInit)
{
   ...
   if (ret == IFX_SUCCESS)
   {
      ...
      case IFX_TAPI_INIT_MODE_VOICE_CODER:
         ...
         if (ret == IFX_SUCCESS &&
             ((pDev->hostDev.nEdspVers & ECMD_VERS_EDSP_PRT)
         ...
         switch (src)
         {
            ...
            case  IFX_TAPI_TONE_SRC_DSP:
               /* disable DTMF / AT generator because it is overlaid
                  with the UTG */
               if (pDev->pSigCh[ch].sig_dtmfgen.bit.en != 0)
               {
...
IFX_return_t TAPI_LL_Phone_CidRx_Start (TAPI_CONNECTION *pChannel)
{
   ...
   {
      ret = Dsp_SetCidRec (pCh, IFX_TRUE);
   }
...
IFX_return_t TAPI_LL_Phone_CidRx_Stop (TAPI_CONNECTION *pChannel)
{
   ...
   if (pDev->pSigCh[pChannel->nChannel].sig_ch.bit.en == IFX_TRUE)
   {
      ret = Dsp_SetCidRec (pCh, IFX_FALSE);
   }
   ...
   if (ch < pDev->nSigCnt)
   {
      ...
      case IFX_TAPI_PKT_EV_OOBPLAY_MUTE:
         ...
         if (ret == IFX_SUCCESS)
         {
            ...
            ret = CmdWrite (pDev, pDev->pSigCh[ch].sig_atd1.value,
                            CMD_SIG_ATD_LEN);
            if (ret == IFX_SUCCESS)
               ret = CmdWrite (pDev, pDev->pSigCh[ch].sig_atd2.value,
                               CMD_SIG_ATD_LEN);
            if (ret == IFX_SUCCESS)
               ret = CmdWrite (pDev, pDev->pSigCh[ch].sig_utd1.value,
                               CMD_SIG_UTD_LEN);
#ifndef VIN_2CPE
            if (ret == IFX_SUCCESS)
               ret = CmdWrite (pDev, pDev->pSigCh[ch].sig_utd2.value,
                               CMD_SIG_UTD_LEN);
#endif /* VIN_2CPE */
         }
      }
...
{
   VINETIC_DEVICE *pDev = (VINETIC_DEVICE *)pChannel->pDevice;
   IFX_uint16_t    pCmd [2] = {0}, pData[28] = {0};
   IFX_int32_t     ret = IFX_SUCCESS;
...
IFX_return_t TAPI_LL_Voice_Enable (TAPI_CONNECTION *pChannel, IFX_uint8_t nMode)
{
   VINETIC_DEVICE  *pDev   = (VINETIC_DEVICE *)pChannel->pDevice;
   FWM_COD_CH      *pCodCh = &pDev->pCodCh[pChannel->nChannel].cod_ch;
   IFX_int32_t ret = IFX_SUCCESS, bSet = IFX_FALSE;
                                       ...
                                       IFX_TAPI_MAP_PHONE_t const *pMap)
{
   ...
   err = Con_ConnectPrepare (&pDev->pAlmCh[nThisCh].signal,
                             &pDev->pAlmCh[pMap->nPhoneCh].signal, 0);
   if (err == IFX_SUCCESS)
      err = Con_ConnectPrepare (&pDev->pAlmCh[pMap->nPhoneCh].signal,
                                          ...
                                          IFX_TAPI_MAP_PHONE_t const *pMap)
{
   ...
   /* connect ALM to SIG */
   err = Con_DisconnectPrepare (&pDev->pAlmCh[nThisCh].signal,
                                &pDev->pAlmCh[pMap->nPhoneCh].signal, 0);
   /* connect SIG to ALM */
   if (err == IFX_SUCCESS)
      err = Con_DisconnectPrepare (&pDev->pAlmCh[pMap->nPhoneCh].signal,
                                   &pDev->pAlmCh[nThisCh].signal, 0);
                                    ...
                                    IFX_TAPI_SIG_DETECTION_t const *pSig)
{
   ...
   if (pSig->sig & (IFX_TAPI_SIG_CNGFAXMASK | IFX_TAPI_SIG_CNGMODMASK))
   {
      /*  UTD1 configured for CNG (1100 Hz) detection */
      /*  UTD2 configured for CNG (1300 Hz) detection  */
      Dsp_UtdConf (pCh, pChannel->TapiMiscData.sigMask &
