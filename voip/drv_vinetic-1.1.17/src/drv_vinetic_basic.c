   ...
   case IFX_TAPI_INIT_MODE_PCM_DSP:
      if (ch < pDev->nSigCnt)
      {
         err = Con_ConnectPrepare (&pDev->pSigCh[ch].signal,
                             &pDev->pPcmCh[ch].signal, REMOTE_SIG_OUT);
      }
      break;
...
IFX_LOCAL IFX_int32_t ALM_baseConf (VINETIC_CHANNEL *pCh, IFX_uint8_t nPath)
{
   ...
   case IFX_TAPI_INIT_MODE_PCM_DSP:
      ...
      if (ch < pDev->nSigCnt)
      {
         Con_ConnectPrepare (&pDev->pSigCh[ch].signal,
                             &pDev->pAlmCh[ch].signal, LOCAL_SIG_OUT);
      }
      else
      {
         SET_ERROR(ERR_NOTSUPPORTED);
        ...
        (pDev->nChipMajorRev == VINETIC_V2x)))
   {
      ...
      if (err == IFX_SUCCESS)
         ...
         if (err == IFX_ERROR &&  pDev->err == ERR_NORESOURCE)
            err = IFX_SUCCESS; /* no NELEC resources, we move forward */
   }
#endif /* VIN_FEAT_VIN_C | VIN_FEAT_VIN_M | VIN_FEAT_VIN_2CPE */
   if (err != IFX_SUCCESS)
   {
...
IFX_LOCAL IFX_int32_t CODER_baseConf (VINETIC_CHANNEL *pCh)
{
   ...
   if (err == IFX_SUCCESS)
   {
      ...
      if (ch < pDev->nSigCnt)
      {
         /* here use signal2 */
         err = Con_ConnectPrepare (&pDev->pSigCh[ch].signal,
                             &pDev->pCodCh[ch].signal, REMOTE_SIG_OUT);
      }
...
IFX_LOCAL IFX_int32_t SIGNL_baseConf (VINETIC_CHANNEL *pCh, IFX_uint8_t nPath)
{
   ...
   switch (nPath)
   {
      ...
      case IFX_TAPI_INIT_MODE_PCM_DSP:
         ...
         {
            err = Con_ConnectPrepare (&pDev->pPcmCh[ch].signal,
                                &pDev->pSigCh[ch].signal, LOCAL_SIG_OUT);
         }
         break;
      case IFX_TAPI_INIT_MODE_PCM_PHONE:
      default:
   ...
   if (err == IFX_SUCCESS)
   {
      ...
      pDev->pSigCh[ch].sig_dtmfgen.value[CMD_HEADER_CNT] = (0x4A00 | ch);
   ...
   if (err == IFX_SUCCESS)
   {
      /* set the default level of the DTMF receiver to -30dB. The firmware
         default is set to -56dB. It is recommended by the firmware group not
         to use a value smaller than -30dB. The "TWIST" value stays unchanged */
      /* set length & cmd 2 */
   ...
   if (err == IFX_SUCCESS)
   {
      ...
      /* EN = 1, ET = 0, AS = 1 */
      pDev->pSigCh[ch].dtmf_rec.value = 0x8010 | ch;
   ...
   if (err == IFX_SUCCESS)
   {
      ...
      pDev->pSigCh[ch].sig_mftd.value[CMD_HEADER_CNT+0] = 0;
      pDev->pSigCh[ch].sig_mftd.bit.resnr = ch;
   }
