...
static int VINETIC_Ioctl(struct inode *inode, struct file *filp,
                         unsigned int nCmd, unsigned long nArgument)
{
   /* We initially assume that file-private data belongs to Vinetic DEVICE data
      (not Vinetic channel data) */
   VINETIC_DEVICE *pDev = (VINETIC_DEVICE *) filp->private_data;
   VINETIC_CHANNEL *pCh  = NULL;
   IFX_int32_t ret = IFX_SUCCESS;
   ...
   if (pDev->nChannel)
   {
      /* assumption wrong (Vinetic channel data), get the VINETIC device and
         channel data */
      pCh = (VINETIC_CHANNEL*)filp->private_data;
   ...
   {
      /* assumption correct, get VINETIC channel data */
      pCh = &pDev->pChannel [0];
      bCtrlDev = IFX_TRUE;
   }

   /* process ioctl command according to type */
   switch (IFXOX_IO_GETMAGIC(nCmd))
   {
      case VINETIC_IOC_MAGIC:
         ret = VINETIC_Common_Ioctl (pCh, nCmd, nArgument, bCtrlDev);
         break;
#ifdef EVALUATION
...
Remarks
   ...
   In interrupt context we do not need to protect us against more interrupts.
   To prevent such missuse an assert is done.
   To prevent that the interrupt is disabled by two different tasks a
   mutex semaphore is used. With this only one task at a time may disable
   the interrupt. When calling the function nested an assert is done.
...
#else
/* to debug IRQ enable/disable mechanism, search for the
   define IRQLOCK_DEBUG in file drv_vinetic_api.h */
IFX_void_t Vinetic_IrqLockDevice_debug (VINETIC_DEVICE *pDev, char *fname, int line)
#endif
{
   vinetic_interrupt_t* pIrq = pDev->pIrq;
   IFX_uint32_t flags;
   ...
   /* by definition this function must not be called in interrupt context */
   if (in_interrupt())
   {
      SET_ERROR(DBG_IRQ_LOCK_IN_IRQ_CONTEXT);
      IFXOS_ASSERT(IFX_FALSE);
      return;
   }
   /* if the driver is used in polling mode -> exit now */
   ...
   if (pIrq != IFX_NULL)
   {
      /* specific interrupt routine is configured for this device */

      /* Wait for the mutex before disabling the irq (this may be blocking
         so we cannot do it with global interrupts already disabled ) */
      IFXOS_MutexLock(pIrq->intAcc);
      /* Here we are protected against concurring tasks but can still be
         interrupted by irqs from the vinetic. When an vinetic interrupt
         hits at exact this place no harm is done because it will be
         completed before we proceede at this point in task context.
         However to prevent interrupts while we disable the irq handling
         all interrupts are now disabled globally for a short period. */
      VIN_DISABLE_IRQGLOBAL(flags);
      /* When the next line fails we are called nested within one task */
      IFXOS_ASSERT(pIrq->bIntEnabled == IFX_TRUE);
      /* invoke board or os routine to disable irq */
      VIN_DISABLE_IRQLINE (pIrq->nIrq);
      /* reset enable flag to show that interrupt line is disabled */
      pIrq->bIntEnabled = IFX_FALSE;
      /* unlock the global interrupts */
      VIN_ENABLE_IRQGLOBAL(flags);
   }
   else
   {
      /* there is no specific interrupt service routine configured for this
...
/* to debug IRQ enable/disable mechanism, search for the
   define IRQLOCK_DEBUG in file drv_vinetic_api.h */
IFX_void_t Vinetic_IrqUnlockDevice_debug (VINETIC_DEVICE *pDev, char *fname, int line)
#endif
{
   vinetic_interrupt_t* pIrq = pDev->pIrq;
   IFX_uint32_t flags;
   ...
   if (pIrq != IFX_NULL)
   {
      /* A specific interrupt routine is configured for this device */
      /* Lock interrupts globally for the following operations */
      VIN_DISABLE_IRQGLOBAL(flags);
      /* When the next line fails we are called nested within one task */
      IFXOS_ASSERT(pIrq->bIntEnabled == IFX_FALSE);
      /* Invoke board or os routine to enable irq */
      VIN_ENABLE_IRQLINE(pIrq->nIrq);
      /* Set enable flag to show that interrupt line is enabled */
      pIrq->bIntEnabled = IFX_TRUE;
      /* Unlock the global interrupts again */
      VIN_ENABLE_IRQGLOBAL(flags);
      /* Return the semaphore. This may immediately  trigger a reschedule so
         it is done last after we reenabled the global interrupts. */
