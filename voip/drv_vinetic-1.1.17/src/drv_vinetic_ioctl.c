...
\param iocmd    - ioctl command
\param ioarg    - ioctl argument
\param bCtrlDev - TRUE if ioctl call through control device's fd,
                  FALSE if ioctl call through channel's fd
                                  ...
                                  IFX_uint32_t ioarg, IFX_boolean_t bCtrlDev)
{
   ...
   */
...
#ifdef LINUX
         VINETIC_IO_DRVCTRL tmp;
         IFXOS_CPY_USR2KERN ((IFX_uint8_t *)&tmp, (IFX_uint8_t *)ioarg,
   ...
   {
      VINETIC_IO_OFFSET_CALIBRATION *pDcOff;
#ifdef LINUX
      VINETIC_IO_OFFSET_CALIBRATION tmp;
      IFXOS_CPY_USR2KERN ((IFX_uint8_t *)&tmp, (IFX_uint8_t *)ioarg,
                          sizeof(VINETIC_IO_OFFSET_CALIBRATION));
      pDcOff = &tmp;
#else
      pDcOff = (VINETIC_IO_OFFSET_CALIBRATION *)ioarg;
#endif
      /* Check for case when targeted channel is the channel
         corresponding to the calling channel's fd */
      if (bCtrlDev == IFX_FALSE && pDcOff->nChannel == 0x80)
         pDcOff->nChannel = pCh->nChannel - 1;

      ret = VINETIC_DC_Offset_Calibration(pDev, pDcOff);
#ifdef LINUX
      IFXOS_CPY_KERN2USR ((IFX_uint8_t *)ioarg, (IFX_uint8_t *)&tmp,
            sizeof(VINETIC_IO_OFFSET_CALIBRATION));
#endif /* LINUX */
   }
   break;
   ...
   ON_IOCTL       (pCh, FIO_VINETIC_LT_DISCHARGE,
                   VINETIC_LT_Discharge, VINETIC_IO_LT_DISCHARGE);
   ON_IOCTL       (pCh, FIO_VINETIC_LT_CHARGE,
                   VINETIC_LT_Charge, VINETIC_IO_LT_CHARGE);
   ON_IOCTL       (pCh, FIO_VINETIC_LT_NETWORK,
                   VINETIC_LT_Network, VINETIC_IO_LT_NETWORK);
#ifdef VIN_V21_SUPPORT
   ...
   ON_IOCTL       (pDev, FIO_VINETIC_MB_ACC_TRACE_READ,
                   VINETIC_MbAccTraceRead, VINETIC_MB_ACC_TRACE_READ);
   case FIO_VINETIC_MB_ACC_TRACE_GETSIZE:
      *((IFX_int32_t*)ioarg) = pDev->pMbAccTrEnd - pDev->pMbAccTrStart;
      break;
   case FIO_VINETIC_MB_ACC_TRACE_SETSIZE:
      ret = VINETIC_MbAccTraceSetSize(pDev, ioarg);
      break;
