...
/*******************************************************************************
Description:
   Status update Event handling function. Is called from interrupt routine.
Arguments:
   pChannel    - handle to TAPI_CONNECTION structure
   signal      - mask of events to be set
   signal_ext  - mask of extended events to be set
Return:
   none
*******************************************************************************/
IFX_void_t TAPI_Signal_Event_Update (TAPI_CONNECTION *pChannel,
                                     IFX_uint32_t signal,
                                     IFX_uint32_t signalExt)
{
   ...
   signalExt &= (pChannel->TapiMiscData.sigMaskExt);
   if (signalExt != 0)
   {
      pChannel->TapiMiscData.signalExt |= signalExt;
   }
   if ((signal != 0) || (signalExt != 0)) {
