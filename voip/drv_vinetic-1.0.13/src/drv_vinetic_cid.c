...
IFX_LOCAL IFX_int32_t setBrsCoef (VINETIC_CHANNEL *pCh, IFX_uint8_t nSize)
{
   ...
   IFX_uint16_t   pCmd [2] = {0}, pCoefs [6] = {0},
                  *pData = pDev->pSigCh[ch].cid_sender;
...
IFX_LOCAL IFX_int32_t PrepareForCid (VINETIC_CHANNEL *pCh)
{
   ...
   pCh->cidSend.nPos     = 0;
   pCh->cidSend.pCmd [0] = CMD1_EOP | nCh;
   pCh->cidSend.pCmd [1] = ECMD_CID_DATA;
   pCh->cidSend.pCmd [2] = 0;
   /* read MRSRE1 */
   /** \todo make use of pDev->regMFSRE1 otherwise modem tone detection and CID
             have a problem */
   err = RegRead (pDev, (CMD1_IOP | nCh), &pCh->cidSend.nCidIrMask, 1, MRSRE1);
   /* unmask CID raising edges interrupts : CIS_ACT, CIS_REQ, CIS_BUF */
   nMaskSet = (pCh->cidSend.nCidIrMask & ~SRE1_CID_MASK);
   if (err == IFX_SUCCESS)
      err = RegWrite(pDev, (CMD1_IOP | nCh), &nMaskSet, 1, MRSRE1);
   ...
   if (err == IFX_SUCCESS)
      err = RegModify (pDev, (CMD1_IOP | nCh), MFSRE1, SRE1_CID_MASK,
                        SRE1_CIS_BUF | SRE1_CIS_REQ);
   /* Set Buffer Request Size (CID Coeffs) */
   if (err == IFX_SUCCESS)
      err = setBrsCoef (pCh, pCh->cidSend.nCidCnt);
   if (err == IFX_SUCCESS)
   {
      /* if phone is actually off-hook, stop every thing for CID 1 */
      if ((pCh->cidSend.nCidType == CID_TYPE_1) &&
...
IFX_LOCAL IFX_int32_t OnCidEnd (VINETIC_CHANNEL *pCh)
{
   ...
   /* write back mask before CID */
   pCmd [0] = (CMD1_IOP | (pCh->nChannel - 1));
   pCmd [1] = (MRSRE1 << 8) | 1;
   pCmd [2] = pCh->cidSend.nCidIrMask;
