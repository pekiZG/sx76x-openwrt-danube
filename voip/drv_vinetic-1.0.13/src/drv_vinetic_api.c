...
IFX_LOCAL IFX_int32_t ObxmlPoll (VINETIC_DEVICE *pDev, IFX_uint8_t* count)
{
   ...
   while (cnt <= OBXML_POLL_LOOP)
   {
      ...
      */
      ...
      cnt++;
#ifdef ASYNC_CMD
      /* here somebody else read the data and it is available in pDev->pCmdData */
      if (pDev->nMbxState & DS_CMDREAD)
         return IFX_SUCCESS;
#endif /* ASYNC_CMD */
   }
...
IFX_int32_t CmdWrite (VINETIC_DEVICE *pDev, IFX_uint16_t* pCmd, IFX_uint8_t count)
{
   ...
   {
#ifdef ASYNC_CMD
      /* the user would like to read data, so tell CmdRead that
         there maybe more data that is read */
      Vinetic_IrqLockDevice(pDev);
      pDev->nMbxState |= DS_CMDREQ;
      pDev->nExpCmdData += count + CMD_HEADER_CNT;
      Vinetic_IrqUnlockDevice(pDev);
#endif /* ASYNC_CMD */
...
#ifdef ASYNC_CMD
   /* increase expected data. This will be decreased in CmdReadIsr
      in case of a read in Isr, or in CmdRead in case of a normal read */
   if (pCmd[0] & CMD1_RD)
   {
      pDev->nMbxState |= DS_CMDREQ;
      pDev->nExpCmdData += (CMD_HEADER_CNT + (pCmd [1] & CMD2_LEN));
   }
   else
   {
      /* clear and fill the length field */
...
IFX_int32_t CmdRead (VINETIC_DEVICE *pDev, IFX_uint16_t* pCmd, IFX_uint16_t* pData, IFX_uint8_t count)
{
   IFX_int32_t err;
#ifdef ASYNC_CMD
   IFX_int32_t i, bServiced = IFX_FALSE;
#endif /* ASYNC_CMD */
   ...
   pCmd[1] |= (IFX_uint16_t)count;
#ifdef ASYNC_CMD
   /* set flag that we requested data */
   pDev->nMbxState |= DS_RDCMD_SENT;
   /* set the expected data amount: data + command header */
   count += CMD_HEADER_CNT;
   pDev->nExpCmdData += count;
#endif /* ASYNC_CMD */
   ...
   pDev->lastCmd[1] = pCmd[1];
#endif /* TESTING */
   /* remove read flag to avoid problems with reusing the command */
   pCmd[0] &= ~CMD1_RD;
   if (err != IFX_SUCCESS)
   {
      SET_ERROR (ERR_DEV_ERR);
   ...
   do
   {
      /* if data was not previously read at a different location */
      if (!(pDev->nMbxState & DS_CMDREAD))
      {
         /* poll for data */
         ...
         if (err != IFX_SUCCESS)
         {
            /* release tasks lock */
            IFXOS_MutexUnlock(pDev->mbxAcc);
            pDev->nMbxState &= ~(DS_CMDREQ | DS_CMDREAD | DS_RDCMD_SENT);
            pDev->nExpCmdData = 0;
            return IFX_ERROR;
         }
         Vinetic_IrqLockDevice(pDev);
         /* data is availabe so read it out */
         pDev->nExpCmdData -= count;
      }
      else
         /* lock now, since we check if data is available. From here till
            read out of data nobody may disturb */
         Vinetic_IrqLockDevice(pDev);
      if (!(pDev->nMbxState & DS_CMDREAD))
      {
         /* There is data to read, signaled by ObmxPoll */
         if (pDev->nMbxState & DS_CMDREQ)
         {
            /* we expect more data then we want to have */
            err = SC_READ (SC_RCOBX, pDev->pCmdData, count);
            pDev->nCmdReadCnt = count;
         ...
         /* signal that command request has been satisfied */
         pDev->nMbxState &= ~(DS_RDCMD_SENT);
         /* release interrupt lock */
         Vinetic_IrqUnlockDevice (pDev);
         if (err != IFX_SUCCESS)
...
/*         if ((pDev->nExpCmdData == 0))
      ...
      /* dispatch the data:
         try to find the requested data and dispatch everything,
         if not previously done. CMDREAD was set when we read the data here.
         CMDREQ was set when data were requested at another location */
      if (pDev->nMbxState & (DS_CMDREQ | DS_CMDREAD))
      {
         i = 0;
         /* unfortunately we have to lock here to be safe from
            other outobox reads. The other possibility would be
            a ring buffer, but this increases the complexity again.
            This approach is choose, because the cases were more data is
            available in the outbox are very few */
         Vinetic_IrqLockDevice(pDev);
         while (i < pDev->nCmdReadCnt)
         {
            if (((pDev->pCmdData[i] & ~CMD1_RD)  == pCmd[0]) &&
                 (pDev->pCmdData[i + 1] == pCmd[1]))
            {
               /* thats ours */
               memcpy (pData, &pDev->pCmdData[i],
                        2 * (pDev->pCmdData[i + 1] & CMD2_LEN) + CMD_HEADER_CNT);
               if (pDev->nExpCmdData == 0)
                  bServiced = IFX_TRUE;
               pDev->nMbxState &= ~DS_RDCMD_SENT;
               if (pDev->nMbxState & DS_CMDREQ)
            ...
            /* there is something of interest, which was requested
               before and not previously dispatched */
            if (pDev->nMbxState & DS_CMDREQ)
            {
               DispatchCmd (pDev, &pDev->pCmdData[i]);
            }
            /* set to next command */
            i += (pDev->pCmdData[i + 1] & CMD2_LEN) + CMD_HEADER_CNT;
         }
         pDev->nCmdReadCnt = 0;
         pDev->nMbxState &= ~(DS_CMDREQ | DS_CMDREAD);
         Vinetic_IrqUnlockDevice(pDev);
      }
   } while (!bServiced);
#else
...
IFX_void_t  CmdReadIsr (VINETIC_DEVICE *pDev)
{
   ...
   for (;;)
   {
      ...
      if (nObxml & OBXML_CDATA)
      {
         ...
         err = SC_READ (SC_RCOBX, pDev->pCmdData + pos , nSize);
         if (err == IFX_ERROR)
            return;
         pos += nSize;
      }
   ...
   if (nSize)
   {
      ...
      /* we read the data, in one case when CmdRead also expects and
         the data is not already in the outobx this field may not be zero */
      pDev->nExpCmdData -= nSize;
      /* if CmdRead already sent a command, now we probable read it out */
