 ...
 ****************************************************************************
   ...
   Remarks     : !whole module still under construction. Implementation of spec
                  TAPI_DS14_Prel7.pdf still ongoing.
*******************************************************************************/
...
IFX_LOCAL IFX_void_t ringpausetimecid_add (TAPI_CONNECTION *pChannel)
{
   ...
   switch (pChannel->TapiCidConfig.nAppearance)
   {
   case CID_AFTER_DELAY:
   case CID_BETWEEN_RINGBURST:
      /* calculate rest of time after initial delay */
      if (pChannel->TapiCidConfig.nAppearance == CID_AFTER_DELAY)
      {
      ...
      if (pCidData->nAdditionalTime > pChannel->TapiCidConfig.nDelay)
      {
         /* wait only the rest of cadence off time */
         pCidData->nAdditionalTime -= pChannel->TapiCidConfig.nDelay;
      }
      else
      {
...
IFX_LOCAL IFX_int32_t ringalertcid_ntt (TAPI_CONNECTION *pChannel)
{
   ...
   case TAPI_RING_STATE_SALERT:
      ...
      if ((pRingData->bIsRingBurstState == IFX_FALSE) &&
          (pChannel->TapiCidTx.bCidAlertEvt == IFX_TRUE))
      {
         /*PRINTF ("NTT offhook: send CID\r\n");*/
         pRingData->nRingTimerState = TAPI_RING_STATE_CID_START;
         TAPI_SetTime_Timer (pRingData->RingTimerID, pChannel->TapiCidConfig.nDelay,
                             IFX_FALSE, IFX_FALSE);
      }
      else if (pChannel->TapiCidTx.nBurstCnt == pChannel->TapiCidConfig.nBurstCount)
      {
...
IFX_LOCAL IFX_int32_t ringalertcid_default (TAPI_CONNECTION *pChannel)
{
   ...
   case CID_BEFORE_RINGBURST:
      pRingData->nRingTimerState = TAPI_RING_STATE_CID_START;
      nTime = pChannel->TapiCidConfig.nDelay;
      pRingData->bIsRingBurstState = IFX_FALSE;
   ...
   case CID_AFTER_DELAY:
      ...
      break;
   case CID_BETWEEN_RINGBURST:
      if ((pChannel->TapiCidTx.nBurstCnt == pChannel->TapiCidConfig.nBurstCount)
          && (pRingData->bIsRingBurstState == IFX_FALSE))
      {
         pRingData->nRingTimerState = TAPI_RING_STATE_CID_START;
         nTime = pChannel->TapiCidConfig.nDelay;
...
#ifdef TAPI_CID
      if ((ret == IFX_SUCCESS) &&
          (pChannel->TapiCidTx.nBurstCnt < pChannel->TapiCidConfig.nBurstCount))
      {
         pChannel->TapiCidTx.nBurstCnt++;
      }
#endif /* TAPI_CID */
   }
   else
...
IFX_void_t TAPI_Phone_Ring_OnTimer(Timer_ID Timer, IFX_int32_t nArg)
{
   ...
   case  TAPI_RING_STATE_INITIAL_RINGING:
      ...
      if ((pChannel->TapiCidTx.bIsNewData == IFX_TRUE) &&
          (pChannel->TapiCidConfig.nAppearance == CID_AFTER_DELAY))
      {
         TAPI_Phone_CID1_OpControl (pChannel, TAPI_CID1_ALERT, NULL,
                                    ringalertcid_default, NULL);
      }
                                  ...
                                  TAPI_PHONE_CID const *pPhoneCid)
{
   ...
   if ((ret == IFX_SUCCESS) && (pPhoneCid != NULL))
   {
      ...
      if (ret == IFX_SUCCESS && pChannel->TapiCidConfig.nSpec != TAPI_CID_NTT)
      {
