                              ...
                              loff_t * ppos)
{
   ...
   /** \todo here the copy is only neccessary for linux. In VxWorks the user
      space could pass a buffer with two leading empty words */
   copy_from_user (&pData[2], buf, (IFX_uint32_t)count);
...
IFX_void_t Vinetic_IrqUnlockDevice (VINETIC_DEVICE *pDev)
{
   ...
   else
   {
      ...
      if (pDev->nIrqMask != 0)
      {
         VIN_ENABLE_IRQGLOBAL(pDev->nIrqMask);
         pDev->nIrqMask = 0;
      }
   }
