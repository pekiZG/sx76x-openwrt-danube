...
IFX_LOCAL IFX_int32_t Utg_SetCoeff (TAPI_SIMPLE_TONE const *pTone, IFX_uint16_t *pCmd);
#endif /* (VIN_CFG_FEATURES & VIN_FEAT_VOICE) */

#if 0
IFX_LOCAL IFX_int32_t SetUTG           (VINETIC_CHANNEL *pCh, IFX_boolean_t bEn, IFX_boolean_t bSM,
                                         IFX_boolean_t bAdder, IFX_uint16_t nA1val, IFX_uint16_t nA2val);
#endif /* 0 */
IFX_LOCAL TAPI_CMPLX_TONE_STATE TG_ToneStep (VINETIC_CHANNEL* pCh, TAPI_SIMPLE_TONE const *pTone, IFX_uint8_t res);
...
IFX_LOCAL IFX_int32_t Utg_SetCoeff (TAPI_SIMPLE_TONE const *pTone, IFX_uint16_t *pCmd)
{
   ...
   for (i = 0; i < TAPI_TONE_MAXSTEPS; ++i)
   {
      ...
      if (pTone->modulation[i] & TAPI_TONE_MODULATION_ON)
   ...
   /* Always set the GO_1 and GO_2 coefficients to -10 dB.
     Formula used to set power level is 32768*10**(-10dB/20) which results
     to a decimal value of 10362 (287A hexadecimal)    */
   pCmd [25] = 10362;
   pCmd [26] = 10362;
   return IFX_SUCCESS;
}
...
IFX_void_t Tone_OnTimer(Timer_ID Timer, IFX_int32_t nArg)
{
   ...
   if (pData->src == TAPI_TONE_SRC_DSP)
   {
      IFX_uint16_t nFibxms;
      VINETIC_DEVICE* pDev = pCh->pParent;

      SC_READ (SC_RFIBXMS, &nFibxms, 1);
      if ((nFibxms & FIBXMS_CBOX) < 31)
      {
   ...
   else if (pData->nType == TAPI_TONE_TYPE_COMP)
   {
      ...
      TAPI_COMPOSED_TONE *pComposedTone;
   ...
   else
   {
      ...
      Tone_UTG_Start(pChannel, pToneSimple, pData->dst);
   }
...
\remark
   This function is called from interrupt level. Is handles only
   tone resource 0
\return
   none
*/
IFX_LOCAL IFX_void_t Event_SimpleTone(TAPI_CONNECTION *pChannel)
{
   ...
   /* in this function resource number 0 is used. Resource 1 is not allowed to use
      this time -> This is still a problem: nArg must specify which tone resource */
   pData = &pChannel->TapiComplexToneData[0];
   ...
   /* An infinite tone sequence repetition was requested
      ...
      Repeat tone sequence again after the voice path timer expires. */
            ...
            pData->nSimpleCurrReps <
            pData->nSimpleMaxReps)
   {
      /* Start the voice path teardowm timer */
      TAPI_SetTime_Timer(pCh->pToneRes[0].Tone_Timer,
                         pData->nPauseTime,
                         IFX_FALSE, IFX_FALSE);
   }
   ...
   else
   {
      ...
      TAPI_Stop_Timer(pCh->pToneRes[0].Tone_Timer);
   }
}
...
\return
   none
*/
IFX_LOCAL IFX_void_t Event_ComposedTone(TAPI_CONNECTION *pChannel)
{
   VINETIC_DEVICE  *pDev = (VINETIC_DEVICE *)pChannel->pDevice;
   VINETIC_CHANNEL *pCh  = &pDev->pChannel[pChannel->nChannel];
   TAPI_TONE_DATA* pData;

   /* in this function resource number 0 (UTG) is used */
   pData = &pChannel->TapiComplexToneData[0];
...
IFX_LOCAL TAPI_CMPLX_TONE_STATE TG_ToneStep(VINETIC_CHANNEL* pCh,
                                 TAPI_SIMPLE_TONE const *pTone, IFX_uint8_t res)
{
   if (pCh->nToneStep > TAPI_TONE_MAXSTEPS ||
       pTone->cadence[pCh->nToneStep] == 0)
   {
      ...
      if (pTone->loop == 0 || pTone->loop > 1)
      {
         ...
         {
            /* set tone timer */
            TAPI_SetTime_Timer (pCh->pToneRes[0].Tone_Timer, pTone->pause,
                                IFX_FALSE, IFX_TRUE);
   ...
   DSCR.value &= ~(DSCR_COR8 | DSCR_PTG | DSCR_TG1_EN | DSCR_TG2_EN);
   if (pTone->frequencies[pCh->nToneStep] & TAPI_TONE_FREQA)
      /* data to start tone generator 1 */
      DSCR.value |= (DSCR_COR8 | DSCR_PTG | DSCR_TG1_EN);
   if (pTone->frequencies[pCh->nToneStep] & TAPI_TONE_FREQB)
...
IFX_int32_t Tone_TG_Play (VINETIC_CHANNEL* pCh)
{
   ...
   if (pTone->frequencies[0] & TAPI_TONE_FREQB)
   {
                            ...
                            TAPI_TONE_DST dst)
{
   ...
   IFX_uint16_t     pCmd [27] = {0};

   /* calling function ensures valid parameter */
   IFXOS_ASSERT(pToneSimple != IFX_NULL);
   /* Tone is already playing */
   if (pChannel->TapiComplexToneData[0].nToneState == TAPI_CT_ACTIVE)
   {
      TRACE(VINETIC, DBG_LEVEL_HIGH,
           ("Tone_UTG_Start: Already started\n\r"));
      return IFX_ERROR;
   }

   memset (pCmd, 0, sizeof(pCmd));
   pCmd [0] = CMD1_EOP | (pCh->nChannel - 1);
   pCmd [1] = ECMD_UTG_COEF;
   /* set the UTG coefficients */
   Utg_SetCoeff (pSimpleTone, pCmd);
   ret = CmdWrite (pDev, pCmd, 25);
   ...
   {
      /* tone is playing now, so set the state */
      pChannel->TapiComplexToneData[0].nToneState = TAPI_CT_ACTIVE;
...
IFX_void_t Tone_EventUTG_Deactivated(TAPI_CONNECTION *pChannel)
{
  ...
  TAPI_TONE_DATA* pData;
   ...
   /* This is a spurious interrupt because no-one has requested tone playing */
