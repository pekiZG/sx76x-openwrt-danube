                                   ...
                                   IFX_uint16_t nSRS1)
{
   ...
   if (nISRS1 & SRS1_HOOK)
   {
      ...
      else
      {
         ...
         if ((pCh->cidSend.nCidCnt) && (pCh->cidSend.nCidType == CID_TYPE_1))
         {
      ...
      /* in case an NTT CID1 is ongoing, this offhook is the aknowledge to the
         NTT alert routine */
      if ((nSRS1 & SRS1_HOOK)                                   &&
          (pCh->pTapiCh->TapiCidConfig.nSpec == TAPI_CID_NTT)   &&
          (pCh->pTapiCh->TapiCidTx.bIsNewData == IFX_TRUE)          &&
          (pCh->pTapiCh->TapiCidTx.bCidAlertEvt == IFX_FALSE))
      {
         pCh->pTapiCh->TapiCidTx.bCidAlertEvt = IFX_TRUE;
      }
      else
#endif /* TAPI_CID */
      {
