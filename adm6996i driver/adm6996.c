...
** 28 FEB 2008  B. Bogaerts	add IGMP functions for Siemens IGMP snooping module
*******************************************************************************/
...
#define CONFIG_PORTA_DANUBE
#define CONFIG_SWITCH_ADM6996_MDIO

int debug=1; //Default=0 DEBUG=5
...
//060620:henryhsu modify for vlan   addr=((addr<<16)|(1<<21))&0x3ff0000;
	...
	*DANUBE_PPE32_ETOP_MDIO_ACC =(0xC0000000|addr);
	...
	// GSU start
	{
		...
		while (((*DANUBE_PPE32_ETOP_MDIO_ACC) & 0x80000000) && (counter != 0)) {
			counter--;
		}
		if (counter == 0) {
			printk ("\nADM6996 SMI read failed.\n");
...
/* IOCTL function */
int adm_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
    PREGRW uREGRW;
    unsigned int rtval;
    ...
    {
        printk("adm_unlocked_ioctl: IOC_TYPE(%x) != ADM_MAGIC(%x)! \n", _IOC_TYPE(cmd), ADM_MAGIC);
        return (-EINVAL);
    }

    if(_IOC_NR(cmd) >= KEY_IOCTL_MAX_KEY)
    {
        printk(KERN_WARNING "adm_unlocked_ioctl: IOC_NR(%x) invalid! \n", _IOC_NR(cmd));
        return (-EINVAL);
    }
...
struct file_operations adm_ops =
{
    ...
    open: adm_open,
    release: adm_release,
    unlocked_ioctl: adm_unlocked_ioctl
};
...
// GSU start
static int
adm_ioctl_regwr_powerdown (unsigned long args)
{
	/* Remark:
	...
	REGRW stREGRW;
	PREGRW uREGRW = &stREGRW;
	unsigned int val = 0xff;
	memcpy (uREGRW, (PREGRW) args, sizeof (REGRW));

	switch (uREGRW->mode) {
	case REG_READ:
		ifx_sw_read (uREGRW->addr, &val);
		uREGRW->value = val;
		memcpy ((PREGRW) args, uREGRW, sizeof (REGRW));
		break;
	case REG_WRITE:
		ifx_sw_write (uREGRW->addr, uREGRW->value);
		break;
...
adm_phy_powerdown (void)
{
	...
	//printk("PowerDown  ADM6996... \n");

	for (stREGRW.addr = 0x0200; stREGRW.addr <= 0x0280;
	     stREGRW.addr += 0x0020) {
		do {
			stREGRW.mode = REG_WRITE;
			stREGRW.value = 0x3900;
			adm_ioctl_regwr_powerdown ((unsigned long) &stREGRW);
			stREGRW.mode = REG_READ;
			adm_ioctl_regwr_powerdown ((unsigned long) &stREGRW);
		}
		while (stREGRW.value != 0x3900);
	}
}
// GSU end
#endif // CONFIG_PORTA_DANUBE

//28022008 B.Bogaerts added some IGMP functions for the Siemens IGMP snooping module.
 void adm_enable_igmp_snooping (void)
	{
	if (debug > 0){ 
		printk(KERN_INFO "Inf ADM6996    : Enable IGMP Hardware Snooping.\n");
	}
	ifx_sw_write(ADM_SW_HISE,0X02);
	ifx_sw_write(ADM_SW_ASC,0X9001);
	}//end enable

void adm_disable_igmp_snooping (void)
	{
	if (debug > 0){ 
		printk(KERN_INFO "Inf ADM6996    : Disable IGMP Hardware Snooping.\n");
	}
	ifx_sw_write(ADM_SW_HISE,0X00);
	}// end disable

//clear membership table
void adm_clear_igmp_table (void)
	{	
	unsigned short igmp_membership;
	...
	control[3]=0;
	if (debug > 0){ 
		printk(KERN_INFO "Inf ADM6996    : Clear IGMP Membership Table.\n");
	}
	
	for(igmp_membership = 0; igmp_membership < 32; ++igmp_membership)
   	{
       		status[5]=0x8000;
		while ( (status[5] & 0x8000) != 0) /* Wait until Access Engine is NOT BUSY */
         	{
         	ifx_sw_read(0x125,&status[5]);
         	}
      		ifx_sw_write(0x11A,control[0]);
      		...
      		ifx_sw_write(0x11C,control[2]);
      		ifx_sw_write(0x11D,control[3]);
      		control[4] = igmp_membership;
      		ifx_sw_write(0x11E,control[4]);
      		control[5] = 0x40;
      		ifx_sw_write(0x11F,control[5]);
   	}
	}// end clear_igmp_table

//add_membership to Membership Table
// entry into table must be between 0 and 32
// port can be from 1 to 4	
// group will be divided into two parameters
// ex. 239.255.0.1 => EF.FF.00.01 => group= 00FF group1=0001
void adm_add_membership (int* entry, unsigned short port, unsigned short group, unsigned short group1)
        {
        unsigned short control[6];
        unsigned int status[6];
        if (debug > 0){
                printk(KERN_INFO "Inf ADM6996    : Add IGMP Membership into Table on entry %02X for port %02x for groupaddress %04X%04X.\n", *entry, port,group,group1);
	}
        //first part of group address
	control[0]=group1;
	switch (port) {
		case PORT1:
			control[1]=IGMP_PORT_1;
			break;
		case PORT2:
			control[1]=IGMP_PORT_2;
			break;
		case PORT3:
			control[1]=IGMP_PORT_3;
			break;
		case PORT4:
			control[1]=IGMP_PORT_4;
			break;
		default:
			control[1]=IGMP_PORT_UNKNOWN;
			break;
	}//end switch
	control[1]=control[1] << 7; //shift portmap 
	control[1]=control[1] | group;
        control[2]=0;
        control[3]=0x0100; // set occupy
        control[4]=*entry;
	control[5]=0x40; // write command
...
void adm_del_membership (int entry)
	{
        unsigned short control[6];
        unsigned int status[6];
	control[0]=0;
	...
	if (debug > 0){
                printk(KERN_INFO "Inf ADM6996    : Delete IGMP Membership into Table on entry %02X.\n", entry);
	}
        //delete entry
	control[4]=entry;
	control[5]=0x40; // write command
...
int adm_find_membership (int* free_entry, unsigned short port, unsigned short group, unsigned short group1)
	{
        unsigned short igmp_membership=0;
        unsigned short control[6];
	...
	*free_entry = -1;
        if (debug > 0){
                printk(KERN_INFO "Inf ADM6996    : Find IGMP Membership into Table on entry %i for port %02x for groupaddress %04X%04X.\n", *free_entry, port,group,group1);
	}
 	
	while ( (result == -1) & (igmp_membership < 32) )	
   	{
	status[5]=0x8000;
      	while ((status[5] & 0x8000) != 0) /* Wait until Access Engine is NOT BUSY */
        	{
	...
	control[4] = igmp_membership;
      	ifx_sw_write(0x11E, control[4]);
      	control[5] = 0x50; // read command
      	ifx_sw_write(0x11F, control[5]);
	status[5]=0x8000;
      	while ((status[5] & 0x8000) != 0) /* Wait until Learning Table NOT BUSY */
         	{
         	ifx_sw_read(0x125,&status[5]);
         	}
      	ifx_sw_read(0x124, &status[4]);
	...
	control[3] = (status[3] & 0x0200) ? 0x0001 : 0x0000; //BAD entry into table
	control[2] = (status[3] & 0x0100) ? 0x0001 : 0x0000; //occupied entry
        control[1] = (status[1] & 0x1F80) >> 7; // portmap
	control[0] = (status[1] & 0x007F); // upper group address

	// Store first free entry into variable
	if ( *free_entry == -1 )
	{
        if (debug > 3){
        printk(KERN_INFO "Dbg ADM6996    : control %04X=Ox0000 %04X=0x0000\n",control[3],control[2]);
	}
	if ((control[3] == 0x0000) & (control[2] == 0x0000))
		{
		if (debug > 3){
                	printk(KERN_INFO "Inf ADM6996    :First free entry %02X.\n", igmp_membership);
		}
		 *free_entry = igmp_membership;
		}
	}	 
	switch (port) {
                case PORT1:
                        control_port=IGMP_PORT_1;
                        break;
                case PORT2:
                        control_port=IGMP_PORT_2;
                        break;
                case PORT3:
                        control_port=IGMP_PORT_3;
                        break;
                case PORT4:
			control_port=IGMP_PORT_4;
                        break;
                default:
                        control_port=IGMP_PORT_UNKNOWN;
                        break;
        }//end switch
	// check of group address 
        if (debug > 3){
        printk(KERN_INFO "Inf ADM6996    : control %04X=%04X %04X=%04X %04X=%04X.\n",control[0],group,status[0], group1,control_port,control[1]);
	}
	if ( (control[0] == group) & (status[0] == group1) & (control_port == control[1]))
	{
        	// found group address for correct port
		if (debug > 0){
                	printk(KERN_INFO "Inf ADM6996    : Membership found on entry %02X.\n", igmp_membership);
		}
		result = igmp_membership;
	}	
...
// end IGMP Functions for Siemens IGMP snooping module.
...
//605112:fchang.removed #ifdef ADM6996_MDC_MDIO_MODE //smi mode //000001.joelin
    ...
    *DANUBE_PPE32_ETOP_MDIO_CFG=0;
    // JR: removed ifx-wlan-drops-patch-2007-11-04: *DANUBE_PPE32_ENET_MAC_CFG &= ~0x18; //ENET0 MAC Configuration
...
// JR: HINT NEVER do this on Porta-Danube it will kill our Flash it' sconnected to HRST...
...
#ifdef CONFIG_PORTA_DANUBE
// GSU start
EXPORT_SYMBOL (adm_phy_powerdown);
// GSU end
#endif // CONFIG_PORTA_DANUBE
...
// Export sympbols B.Bogaerts
EXPORT_SYMBOL (adm_enable_igmp_snooping);
EXPORT_SYMBOL (adm_disable_igmp_snooping);
EXPORT_SYMBOL (adm_clear_igmp_table);
EXPORT_SYMBOL (adm_add_membership);
EXPORT_SYMBOL (adm_del_membership);
EXPORT_SYMBOL (adm_find_membership);
// end IGMP proxy/snoopping
